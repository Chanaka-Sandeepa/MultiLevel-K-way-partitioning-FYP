...
def random_match(graph):
    ...
    while (graph.number_of_nodes() > node_limit):
        ...
        vertex1 = choice(list(graph.nodes()))
        vertex2 = choice(list(graph.nodes()))
        if not vertex1 == vertex2:
            vertex1_coordinate = get_node_coordinates(node_list, vertex1)
            vertex2_coordinate = get_node_coordinates(node_list, vertex2)
            if ((CSVReader.getDistanceFromCoordinate(vertex1_coordinate, vertex2_coordinate) < 10000)):
                print("ddadada")
                graph = merge_nodes(graph, vertex1, vertex2, vertex1_coordinate)
                matched_nodes.append(vertex1)
                matched_nodes.append(vertex2)
